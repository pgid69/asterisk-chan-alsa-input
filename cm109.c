...
static void cm109_urb_irq_callback(struct urb *urb)
{
   ...
   u8 special;
	...
	/* Special keys */
   special = dev->irq_data->byte[HID_IR0] & 0x0f;
	if (special) {
      size_t i;
      u8 mask;
      for (i = 0, mask = 0x01; (i < 4); i += 1, mask <<= 1) {
         if (special & mask) {
            report_key(dev, dev->keymap[0xff + mask]);
            report_key(dev, KEY_RESERVED);
         }
      }
	}
	...
	if (dev->keybit == 0xf) {
      ...
      const int code = dev->irq_data->byte[HID_IR1];
		report_key(dev, dev->keymap[code]);
